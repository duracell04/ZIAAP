...
export function buildDemonstrationDossier(state: ContractState) {
  ...
  const artifacts: DossierArtifact[] = [
    ...
    { ...common, id: "stress-report", index: 5, title: "Scenario Laboratory Report", artifactType: "calibration_result", actor: stressExecutionStatus === "executed_unverified" ? "Declared live model" : "Concept curator", executionStatus: stressExecutionStatus, version: `Constitution v${state.constitution.version}`, provenance: "Four selected synthetic Scenario Laboratory artifacts", consequence: "Supports simulated acknowledgement only; does not independently validate", available: stressResults.length === state.calibrationScenarios.length, summary: `${stressResults.length} of ${state.calibrationScenarios.length} scenario artifacts available.`, details: state.calibrationScenarios.map((scenario) => ({ label: scenario.title, value: scenario.result ? `${scenario.result.executionStatus}: ${scenario.result.outcome}. ${scenario.result.limitations.join(" ")}` : "Not executed" })) },
  ...
  return {
    title: "ZIAAP Synthetic Audit Dossier",
