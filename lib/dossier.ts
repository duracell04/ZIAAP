...
export function buildDemonstrationDossier(state: ContractState) {
  ...
  const artifacts: DossierArtifact[] = [
    ...
    { ...common, id: "stress-report", index: 5, title: "Configuration Scenario-Test Report", artifactType: "calibration_result", actor: stressExecutionStatus === "executed_unverified" ? "Declared live model" : "Concept curator", executionStatus: stressExecutionStatus, version: `Constitution v${state.constitution.version}`, provenance: "Four selected synthetic Gate 2 scenario-test artifacts", consequence: "Supports simulated acknowledgement only; does not independently validate", available: stressResults.length === state.calibrationScenarios.length, summary: `${stressResults.length} of ${state.calibrationScenarios.length} scenario artifacts available.`, details: state.calibrationScenarios.map((scenario) => ({ label: scenario.title, value: scenario.result ? `${scenario.result.executionStatus}: ${scenario.result.outcome}. ${scenario.result.limitations.join(" ")}` : "Not executed" })) },
    { ...common, id: "protocol-manifest", index: 6, title: "Configuration Manifest", artifactType: "exact_protocol_manifest", actor: "Concept transition verifier", executionStatus: "not_executed", version: `Manifest v${state.appointment.manifestVersion}`, provenance: "Canonical selected protocol configuration and SHA-256 digest", consequence: "Supports internal simulation consistency and stale-state detection", available: Boolean(state.appointment.manifestHash), summary: state.appointment.manifestHash ?? "Manifest not prepared.", details: [{ label: "SHA-256", value: state.appointment.manifestHash ?? "Pending" }, { label: "Canonical contents", value: text(manifest) }, { label: "Boundary", value: "Not a build, dependency, provider, environment, runtime, identity, or signature attestation." }] },
    ...
    { ...common, id: "case-production", index: 8, title: "Synthetic Case-Production Record", artifactType: "synthetic_merits_record", actor: "Supplier and customer · simulated", executionStatus: "not_executed", version: state.dispute.id, provenance: "Curated synthetic claim, defence, evidence, objections, and missing evidence", consequence: "Defines the shared merits record bound to the manifest", available: Boolean(state.dispute.appointmentHash), summary: state.dispute.claim, details: [{ label: "Defence", value: state.dispute.defence }, { label: "Shared evidence", value: state.dispute.sharedEvidence.join(" · ") }, { label: "Missing evidence", value: state.dispute.missingEvidence.join(" · ") }] },
    ...
    { ...common, id: "reasoning-memorandum", index: 10, title: "Advisory Reasoning Memorandum", artifactType: "reasoning_memorandum", actor: state.reasoningMemorandum?.metadata.executionMode === "live" ? "Declared live model" : "Concept curator", executionStatus: state.reasoningMemorandum?.metadata.executionStatus ?? "not_executed", version: state.reasoningMemorandum?.metadata.artifactId ?? "Pending", provenance: state.reasoningMemorandum?.metadata.provenance ?? "No artifact selected", consequence: "State-derived advisory input to fictional human review; never a verdict or award", available: Boolean(state.reasoningMemorandum), summary: state.reasoningMemorandum?.proposedDisposition ?? "Reasoning memorandum not yet produced.", details: [{ label: "Reasoning summary", value: state.reasoningMemorandum?.reasoningSummary ?? "Pending" }, { label: "Uncertainty", value: state.reasoningMemorandum?.uncertainty.join(" · ") ?? "Pending" }, { label: "Escalation", value: state.reasoningMemorandum?.escalationFlags.join(" · ") ?? "Pending" }] },
    { ...common, id: "human-decision", index: 11, title: "Simulated Human Decision Record", artifactType: "human_decision", actor: state.constitution.humanArbitrator.name, executionStatus: "not_executed", version: "Human simulation record v1", provenance: "Fictional independent pre-assessment, rationale, and review checklist", consequence: "Demonstrates human control; creates no signature, award, or legal effect", available: Boolean(state.humanDecision.simulatedDecisionRecord), summary: state.humanDecision.simulatedDecisionRecord ? `${state.humanDecision.status}: ${state.humanDecision.rationale}` : "Fictional human decision not yet recorded.", details: [{ label: "Pre-assessment", value: state.humanDecision.preliminaryAssessment || "Pending" }, { label: "Disposition", value: state.humanDecision.status }, { label: "Checklist", value: text(state.humanDecision.checklist) }] },
    { ...common, id: "authority-summary", index: 12, title: "Authority and Limitation Summary", artifactType: "authority_summary", actor: "ZIAAP concept demonstrator", executionStatus: "not_executed", version: "C0 concept", provenance: "Canonical authority model and artifact metadata", consequence: "Prevents simulation, execution, validation, and legal authority from being conflated", available: true, summary: "Synthetic · simulation_only · legalEffect false · external validation pending.", details: [{ label: "Unavailable statuses", value: "authoritative and validated are reserved and unreachable" }, { label: "No production properties", value: "No identity verification, institutional appointment, production signature, operative award, runtime attestation, or enforcement." }, { label: "Next gate", value: "Gate C1 requires real comprehension and commercial evidence." }] },
  ];

  return {
    title: "ZIAAP Simulated Outcome & Procedural Black Box",
