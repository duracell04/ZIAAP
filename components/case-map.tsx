...
export function ConstitutionBuilder({ state, updatePrinciple, acknowledgeConstitution }: Props) {
  ...
  return <>
    <div className="page-intro"><span>Stage 2 · arbitral reasoning calibration</span><h1>Configure how the protocol should reason—before testing what it does.</h1><p>Calibration sets relationship-specific inference instructions, sources, tools, safeguards, and boundaries. It does not train or fine-tune the underlying model; model weights remain unchanged.</p></div>
    ...
    <div className="definition-banner"><Scale size={21} /><div><strong>Human authority remains external to the protocol</strong><p>A properly appointed human could conduct proceedings through an acknowledged ZIAAP protocol. This showcase uses a fictional record, simulation only, and makes no appointment.</p></div></div>
