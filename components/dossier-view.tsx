...
export function DossierView({ state }: { state: ContractState }) {
  ...
  return <>
    <div className="page-intro dossier-intro"><span>Final demonstration dossier</span><h1>One state-derived record of the complete synthetic journey.</h1><p>Twelve artifacts make the commercial, procedural, technical, and authority story inspectable. Missing lifecycle evidence remains visibly pending; the dossier never invents completion.</p></div>
    ...
    <Card className="dossier-cover"><div><span>ZIAAP · REVIEW CANDIDATE</span><h2>{dossier.title}</h2><p>AI-native dispute governance and arbitration · synthetic · simulation-only · no legal effect</p></div><div className="dossier-score"><strong>{dossier.availableCount}</strong><span>of 12 artifacts available</span></div></Card>

    <div className="dossier-summary"><Card><BookOpenCheck size={20} /><span>Commercial story</span><h2>Align before conflict.</h2><p>Expose divergent expectations, compare options, confirm exact language, and generate an Alignment Annex.</p></Card><Card><FileText size={20} /><span>Protocol story</span><h2>Configure, test, and identify.</h2><p>Calibrate inference-time rules, inspect stress behavior, and hash the Exact protocol manifest.</p></Card><Card><ShieldAlert size={20} /><span>Authority story</span><h2>Simulation never becomes authority.</h2><p>Human control, settlement separation, and legal-effect boundaries remain visible throughout.</p></Card></div>
    ...
    <Card className="external-validation-note"><ShieldAlert size={20} /><div><Badge tone="amber">External validation pending</Badge><h2>Review candidate, not validated release.</h2><p>No expert reviewer participation, comprehension result, usability result, or independent evaluation is claimed. Those questions belong to the next cycle.</p></div></Card>
  </>;
}
