...
export function AppointmentCeremony({ state, freezePackage, confirmHash, setReviewFlag, appoint }: Props) {
  ...
  return <>
    <div className="page-intro"><span>Stage 4 · version-lock and appoint</span><h1>Freeze the validated runtime.</h1><p>The package binds the agreed contract state, Constitution, protocol identity, and approved validation results to one tamper-evident hash.</p></div>
    <div className="readiness-grid"><Card><Badge tone={alignmentReady ? "green" : "amber"}>{alignmentReady ? "Ready" : "Incomplete"}</Badge><h2>Party alignment</h2><p>Both party profiles and every exact clause version confirmed.</p></Card><Card><Badge tone={allCalibrationApproved(state.calibrationScenarios) ? "green" : "amber"}>{allCalibrationApproved(state.calibrationScenarios) ? "Ready" : "Incomplete"}</Badge><h2>Stress-test validation</h2><p>All four safeguards passed and behavior approved by both parties.</p></Card></div>
